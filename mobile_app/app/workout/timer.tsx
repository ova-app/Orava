...
export default function TimerScreen() {
  ...
  // ORA-041 — respecter le toggle Vibrations des settings (avant : Vibration.vibrate brut le contournait).
  const vibrationEnabledRef = useRef<boolean>(true)
  useEffect(() => {
    void AsyncStorage.getItem('settings_vibration').then((v) => {
      vibrationEnabledRef.current = v !== 'false'
    })
  }, [])
  ...
  const triggerFinish = useCallback(() => {
    ...
    clearTick()
    setRemaining(0)
    // ORA-041 — haptique opt-outable (expo-haptics) au lieu de Vibration brut qui ignorait le toggle.
