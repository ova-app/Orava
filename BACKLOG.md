...
### Tests — lot du 23/06/2026 — 🟡 ORA-045 partiel (logique) + gate réparé
- 🟡 **ORA-045 (partiel — logique pure/async)** — 7 suites ajoutées, **import réel uniquement** (zéro recopie, le reproche central du ticket) : `weights`, `muscles`, `feedSignal`, `profileBio`, `featuredPhoto` (5 modules à **0 test** → testés) + `claimsAsync` / `featuredPrAsync` (15 fonctions async claims + pin/clear/hide PR vedette via mock « builder thenable »). **262 → 374 tests**, lint + tsc clean, zéro ligne de prod touchée. **Reste** (hors « safe en parallèle », exige `npm install` + outil natif) : render tests `@testing-library/react-native` sur session/summary + smoke e2e Maestro (login → log set → save).
- ✅ **Coverage gate réparé** (était déjà ROUGE avant ce lot, vérifié en isolant les anciens tests) : les seuils ORA-074 de `claims.ts` (18 %) et `featuredPr.ts` (13 %) avaient dérivé au-dessus du réel (14,7 % / 12,3 %) + global branches/functions sous plancher. Les nouveaux tests repassent tout au vert **sans toucher `package.json`** : `claims.ts` 14,7 % → **67,4 %**, `featuredPr.ts` 12,3 % → **37,7 %**.
...
> ✅ **Code livré** (23/06/2026) : écran `app/delete-account.tsx` (friction « SUPPRIMER » + purge Storage client) + entrée settings COMPTE. RPC `supabase/planned/ora001_delete_account.sql` (`SECURITY DEFINER`) — **⚠️ à appliquer** (SQL Editor). Prérequis : `gyms.created_by` nullable.
...
> ✅ **Livré** (23/06/2026) : écran `app/privacy.tsx` (mention santé art. 9) ; consentement bloquant à l'inscription (`register.tsx`) ; PostHog `defaultOptIn: false` (`_layout.tsx` + `lib/analytics.ts`) + toggle opt-in settings › Confidentialité ; export JSON via `lib/dataExport.ts` (Share natif). 100 % client, aucune migration. *(Upgrade futur export : `expo-file-system` + `expo-sharing`.)*
...
- 🟡 **ORA-045 · [TESTS] 0 % composant / intégration / e2e.** `__tests__/` = tests de logique pure, dont certains **recopient** la fonction testée (`prsBuildPodium.test.ts:12-127`, `sessionUx.test.ts:34-58`) → testent une copie, pas le prod. `@testing-library/react-native`, Detox/Maestro absents. **Action :** render tests sur session/summary + smoke e2e Maestro (login → log set → save) ; tester le code importé. *→ Confirmé empiriquement par le coverage gate (ORA-074) : les tests recopiés n'apparaissent pas dans le rapport ; `predictor.ts` à 12 %.* **Avancé le 23/06** : 7 suites par import réel ajoutées (`weights`/`muscles`/`feedSignal`/`profileBio`/`featuredPhoto` + `claimsAsync`/`featuredPrAsync`), 262 → 374 tests, gate ORA-074 réparé (`claims.ts` 67 %, `featuredPr.ts` 38 %). **Reste à faire** : render tests RN + e2e Maestro (besoin `npm install` + outil natif) ; et retirer/remplacer les 2 tests qui recopient encore le prod (`prsBuildPodium`, `sessionUx` — fonctions inline non exportées, exigerait un export prod).
